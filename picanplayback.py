...
- Script will ask for log file selection, and
    filter/frame (#) will stream all frames "2e1#" will only stream 2e1
...
def send_next_frame(client, csvfile,frame):
    ...
    if line:
            if frame in line:
                ...
                print("sending:", "can "+newvalues[0]+"#", newvalues[1])
...
def main():
    ...
    frame = input("Please enter filter: ")
    with open(filename) as csvfile:
        _ = csvfile.readline()
        print(_)
        my_address = get_local_ip()
        server = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        server.bind((my_address, port)) #<<You can manully overide "my_address" if needed>>
